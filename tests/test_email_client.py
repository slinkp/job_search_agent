...
class TestGmailRepliesSearcher(unittest.TestCase):
    
    @patch('email_client.build')
    def test_send_reply(self, mock_build):
        # Setup mock service
        ...
        self.assertEqual(send_args['userId'], 'me')
        self.assertIn('raw', send_args['body'])
        self.assertEqual(send_args['body']['threadId'], 'test-thread')
